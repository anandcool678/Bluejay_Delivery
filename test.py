...
# Function to load the Excel file and perform the required analysis
def analyze_excel_file(file_path):
    try:
        ...
        for index, row in df.iterrows():
            ...
            if employee_name in consecutive_employees:
                continue
        ...
        for index, row in df.iterrows():
            ...
            if employee_name in short_break_employees:
                continue
            ...
            if employee_name in employee_breaks:
                ...
                if isinstance(time_in, str) and isinstance(last_time_out, str):
                    ...
                    if 1 < time_diff < 10:
                        print(f"Employee: {employee_name}, Position: {position_id}")
                        short_break_employees.add(employee_name)
                else:
                    time_in = None
        ...
        for index, row in df.iterrows():
            ...
            if employee_name in long_shift_employees:
                continue
            ...
            if duration is not None and duration.total_seconds() / 3600 > 14:
                print(f"Employee: {employee_name}, Position: {position_id}")
                long_shift_employees.add(employee_name)
...
if __name__ == "__main__":
    ...
    # Call the analyze_excel_file function for analysis
